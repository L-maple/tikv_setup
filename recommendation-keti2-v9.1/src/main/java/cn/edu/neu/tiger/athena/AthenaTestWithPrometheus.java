...
public class AthenaTestWithPrometheus {
...
//        probability.map((res) -> {
//            String[] splits = String.valueOf(res.getField(0)).split(":");
//            logger.info(String.format("User[%s] ts[%s]", splits[0], splits[2]));
//            return null;
//        }).disableChaining();
        ...
        recommendation.map((res) -> {
            ...
            logger.info(String.format("User[%s] delay[%d - %d = %d] rec: [%s]",
                    res.f0, cur, res.f2, cur - res.f2, String.join(", ", res.f1)));
            return null;
        }).disableChaining();
        streamEnv.execute();
    }
